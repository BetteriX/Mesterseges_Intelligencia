...
class Jatek:
    ...
    def rakovetkezo(self, allapot):
        return [
            (lepes, self.lep(lepes, allapot)) for lepes in self.legalis_lepesek(allapot)
        ]
...
def random_jatekos(jatek: Jatek, allapot):
...
class TicTacToe(Jatek):
    ...
    def __init__(self, h=3, v=3, k=3):
        ...
        self.kezdo = Struct(kovetkezik="X", eredmeny=0, tabla={}, lepesek=lepesek)
    ...
    def ertekel(self, tabla, lepes, jatekos):
        ...
        if (
            self.k_egy_sorban(tabla, lepes, jatekos, (0, 1))
            or self.k_egy_sorban(tabla, lepes, jatekos, (1, 0))
            or self.k_egy_sorban(tabla, lepes, jatekos, (1, -1))
            or self.k_egy_sorban(tabla, lepes, jatekos, (1, 1))
        ):
...
""" Nem tom hogy ezzeket meg kell-e tanulni"""
