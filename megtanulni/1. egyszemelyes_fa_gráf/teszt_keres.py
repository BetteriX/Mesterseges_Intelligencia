...
class Csúcs:
    ...
    def kiterjeszt(self, feladat: Feladat):
...
def mélységi_gráfkereső(feladat: Feladat):
    return gráfkereső(feladat, Verem())


def best_first(feladat: Feladat, heurisztika):
    return gráfkereső(feladat, RendezettLista(heurisztika))


def a_csillag(feladat: Feladat, heurisztika):
    def f(csúcs):
