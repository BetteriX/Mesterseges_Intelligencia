...
class TicTacToe(Game):
    ...
    def result(self, board, step, player):
        ...
        if (
            self.check_triples(board, step, player, (0, 1))
            or self.check_triples(board, step, player, (1, 0))
            or self.check_triples(board, step, player, (1, -1))
            or self.check_triples(board, step, player, (1, 1))
        ):
...
def play_episodes(
    ...
    for episode in tqdm(range(n_episodes)):
        ...
        while not done:
            ...
            if learn:
                agent.learn(
                    state_to_id[state_representation],
                    action,
                    reward,
                    state_to_id[new_state_representation],
                    gamma,
                )
            total_reward += reward
...
while not done:
    # Q-learning agent's turn
    action = agent.act(state=state_to_id[state_representation], epsilon=0)
